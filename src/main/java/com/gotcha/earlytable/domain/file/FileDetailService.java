...
public class FileDetailService extends s3 {
     ...
     */
    @Transactional
    public List<FileDetail> createImageFiles(List<MultipartFile> imageFiles, File file) {
        ...
        try {
            for (int i = 0; i < imageFiles.size(); i++) {
                ...
                // 파일 생성
                FileDetail fileDetail =
                        FileDetail.toEntity(fileStatus, file, fileSeq, FileType.valueOf(extension.toUpperCase()), fileDetailDto);
        ...
        fileDetailRepository.saveAllAndFlush(fileDetails);
     ...
     * 이미지 업데이트 메서드
     *
     * @param fileList 새롭게 추가하는 파일들 / 기존 파일은 포함 안됨
     * @param fileUrlList 현재 수정하려는 파일들의 순서 정렬된 파일 이름 리스트 (기존 파일 url + 새로운 파일 이름의 리스트)
     * @param file
     */
    @Transactional
    public void updateFileDetail(List<MultipartFile> fileList, List<String> fileUrlList, File file) {
        ...
        if (fileList != null && !fileList.get(0).isEmpty()) {
            // 기존의 파일 리스트에 새로운 파일 추가
            List<FileDetail> newFileDetailList = createImageFiles(fileList, file);

            // 파일 이름을 URL로 변경을 위한 map 생성
            Map<String, String> fileNameToUrlMap = newFileDetailList.stream()
        ...
        List<FileDetail> removeFileDetailList = new ArrayList<>();

        // 삭제할 파일 처리
        for(FileDetail fileDetail : fileDetailList) {
            String fileUrl = fileDetail.getFileUrl();
            if (!fileUrlList.contains(fileUrl)) {
                removeFileDetailList.add(fileDetail);
                deleteFile(fileUrl);
            }
        }

        // 직접 파일에서 제외 (고아가 되면서 트랜젝션 끝날 때 삭제됨
        for (FileDetail fileDetail : removeFileDetailList) {
            file.getFileDetailList().remove(fileDetail);
        }
    ...
    public static boolean isImageExtension(String extension) {
        ...
        return Arrays.stream(FileType.values()).noneMatch(fileType -> fileType.getExtension().equalsIgnoreCase(extension));
    }
